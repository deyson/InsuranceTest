...
namespace InsurancePolicy.Web.Providers
{
    public class ApplicationAuthenticationHandler : DelegatingHandler
    {
        ...
        protected override System.Threading.Tasks.Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, System.Threading.CancellationToken cancellationToken)
        {
            ...
            IEnumerable<string> insuranceApiKeyHeaderValues = null;

            // Checking the Header values
            if (request.Headers.TryGetValues("X-InsuranceAppApiKey", out insuranceApiKeyHeaderValues))
            {
                string[] apiKeyHeaderValue = insuranceApiKeyHeaderValues.First().Split(':');
                ...
                if (apiKeyHeaderValue.Length == 2)
                {
                    ...
                    if (appID.Equals("InsuranceUI") && AppKey.Equals("ThisInsuranceIsPersist"))
                    {

                        var userNameClaim = new Claim(ClaimTypes.Name, appID);
                        var identity = new ClaimsIdentity(new[] { userNameClaim }, "InsuranceAppApiKey");
